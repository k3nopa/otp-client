...
class Client(object):
    ...
    **IMPORTANT** the required function signature for a callback can differ
    ...
    def __init__(self, client_id="", otp=False, secret="", path="", layer="", clean_session=None, userdata=None,
    ...
    def _otp_recv(self, bufsize):
        recv_byte = self._sock.recv(bufsize)
        dec_data = onetimepad.decrypt(recv_byte.hex(), self.secret)
        data = binascii.unhexlify(dec_data)
        # print("[Recv] ", recv_byte, data)
        return data

    def _sock_recv(self, bufsize):
        try:
            if self.otp :
                data = self._otp_recv(bufsize)
                return data
            else: 
    ...
    def _otp_send(self, buf):
        enc_data = onetimepad.encrypt(buf.hex(), self.secret)
        data = binascii.unhexlify(enc_data)
        # print("[Send] ", enc_data, data)
    ...
    def _key_exchange(self, sock):
        data = f"{self.path}".encode()

        sock.send(f"{len(data)}".encode())
        time.sleep(.2)
        sock.send(data)

        time.sleep(.2)

        data = f"{self.layer}".encode()
    ...
    def _packet_read(self):
        ...
        while self._in_packet['to_process'] > 0:
            try:
                # TODO: change recv buf based on OTP
                #data = self._sock_recv(self._in_packet['to_process'])
                data = self._sock_recv(self._in_packet['to_process']*2)
            except BlockingIOError:
                return MQTT_ERR_AGAIN
            except ConnectionError as err:
                self._easy_log(
